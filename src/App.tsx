...
import { Language, detectLanguage, translations } from './lib/localization';
...
export default function App() {
  ...
  const [language, setLanguage] = useState<Language>(() => {
    const cached = localStorage.getItem('paper_planner_lang');
    if (cached === 'en' || cached === 'zh-TW') return cached;
    return detectLanguage();
  });

  const handleLanguageChange = (lang: Language) => {
    setLanguage(lang);
    localStorage.setItem('paper_planner_lang', lang);
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-300 ${getThemeWrapperClass()}`} id="app-wrapper">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-6 pb-28 md:pb-24">
        ...
        {activeTab === 'insights' && (
          <InsightsView state={state} theme={theme} language={language} />
        )}
      ...
      <footer className="fixed bottom-0 left-0 right-0 border-t border-inherit bg-opacity-95 backdrop-blur py-3 px-4 z-40" style={{ backgroundColor: 'inherit' }}>
        <div className="max-w-md mx-auto bg-white dark:bg-stone-950 border border-inherit rounded-2xl flex justify-around p-1 shadow-md">
          {[
            { id: 'today', labelKey: 'tab.today', icon: Calendar },
            { id: 'calendar', labelKey: 'tab.calendar', icon: CalendarDays },
            { id: 'history', labelKey: 'tab.history', icon: BookOpen },
            { id: 'insights', labelKey: 'tab.insights', icon: BarChart3 },
            { id: 'settings', labelKey: 'tab.settings', icon: Settings },
          ].map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            const label = translations[language][item.labelKey as keyof typeof translations['en']] || item.labelKey;
            return (
              <button
                key={item.id}
              ...
              >
                ...
                <span className="text-[10px] tracking-wide font-mono font-medium">{label}</span>
              </button>
            );
          })}
        </div>
